...
export default function AdminJobsPage() {
    ...
    const fetchJobs = useCallback(async () => {
        setLoading(true);
        try {
            const res = await adminApi.getJobs({ search, type: "FULL_TIME", page });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setForm({ title: "", companyId: "", location: "", description: "", applyUrl: "", salaryMin: "", salaryMax: "", isRemote: false, isStartup: false, isHFT: false });
            setShowForm(false);
            fetchJobs();
        } catch {} finally {
            setSaving(false);
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-bold font-manrope">Full-Time Jobs ({total})</h1>
            ...
            {showForm && (
                <form onSubmit={handleSubmit} className="border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 mb-6 space-y-3">
                    ...
                    <div className="flex gap-4">
                        ...
                        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isHFT} onChange={(e) => setForm((p) => ({ ...p, isHFT: e.target.checked }))} /> HFT</label>
                    </div>
                    <button type="submit" disabled={saving} className="px-4 py-2 bg-blue-500 text-white text-sm rounded-lg hover:bg-blue-600 disabled:opacity-50">{saving ? "Saving..." : "Create Job"}</button>
                </form>
            )}
                    ...
                    {jobs.map((job: any) => (
                        <div key={job.id} className="border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 space-y-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                                ...
                                {job.isStartup && <span className="text-xs bg-orange-50 dark:bg-orange-900/20 text-orange-600 px-2 py-0.5 rounded">Startup</span>}
                                {job.isHFT && <span className="text-xs bg-purple-50 dark:bg-purple-900/20 text-purple-600 px-2 py-0.5 rounded">HFT</span>}
                            </div>
